	    ...
	    int *cmd_type)		/* return type; or NULL */
{
   ...
   char cmd_copy[256];			/* modifiable copy of cmd. Too large (e.g. MSG_SIZE) crashes vxWorks */
   ...
   while((tok = strtok(cmd_str, " \t")) != NULL) {
	 ...
	 if (strstr(cmd, "STATUS") == 0 &&
	     strcmp(cmd, "SEM.SHOW") != 0 &&
	     strcmp(cmd, "version") != 0) {
	    sendStatusMsg_S(uid, cid, INFORMATION_CODE, 1, "commandIn", cmd);
	 }
      }
